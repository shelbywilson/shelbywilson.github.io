...
while (offset.length < rows * perRow) {
    offset.push(mapRange(Math.random(), 0, 1, -1, 1))
...
export const Daffodils = () => {
    if (WEBGL.isWebGLAvailable()) {
        ...
        const camera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 1, 16);
        let flowers = [];
        let focusPoints = [];
        let petals = [];
        let stats;
        ...
        for (let i = 0; i < (rows * perRow); i += 1) {
            ...
            let trumpetGeom = new THREE.SphereBufferGeometry(0.37, 64, 32, 0, 2*Math.PI, 0, Math.PI/3)//THREE.CylinderGeometry(0.33, 0.33, 0.2, 20);
            trumpetGeom.translate(0, -0.1, 0);
            ...
            daffodilFlower.position.set(0.16, 0.09, -3)
            ...
            daffodil.position.set(
                mapRange(x % perRow, 0, perRow - 1, -perRow, perRow), // + (x%2) * 1.75 + ((x%rows)%2 * -1),
                mapRange(x % rows, 0, rows - 1, rows + 1.15, -rows - 1.15) + offset[x],
